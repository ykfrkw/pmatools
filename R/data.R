#' CBT-I for Depression Response: 18-trial extracted dataset
#'
...
#' the binary outcome.
#'
#' The data are real: study names, sample sizes, and event counts are the
#' values each trial actually reported, as extracted for the source review.
#' Seventeen trials report the outcome and enter the pooled estimate; the
#' remaining trial (Lopez2019) did not report this outcome, so it carries
#' \code{d_r = NA} and is therefore a genuine missing-results case for the
#' publication-bias / ROB-ME workflow.
#'
...
#'   (the lean analysis subset; the full extraction with all variables is
#'   bundled as \code{inst/extdata/cbti_depression.csv}):
#' \describe{
...
#'   \item{rob_d}{Cochrane RoB 2 judgment for the depression outcome:
#'     \code{"L"} low risk of bias, \code{"S"} some concerns, \code{"H"} high
#'     risk of bias -- the three levels RoB 2 defines.}
#' }
#'
#' @examples
