...
# arguments) live in R/step3_threshold.R. The publication-bias wizard's server
# half - reactive, so it could not follow that file's no-session rule - lives
# in R/step3_pubias.R and is called from step3_server() below.
...
step3_server <- function(input, output, session, state) {
  ...
  # The whole of it - the derived node, the breadcrumb, the question cards,
  # the reference plots and their status dots - moved to R/step3_pubias.R.
  # Called from here, at the position the block used to occupy, so the order
  # outputs are registered in is unchanged. It is a plain function, not a
  # module: every input and output id stays bare (see that file's header).
  #
  # pubias_reopen comes back because state$step3_reset() above clears it: the
  # breadcrumb click is the wizard's one piece of stored state, and a click
  # left over from the previous outcome must not decide which question the
  # next one opens on.
  pubias_reopen <- step3_pubias_server(
    input, output, session, state,
    threshold_abs_state      = threshold_abs_state,
    threshold_baseline_state = threshold_baseline_state,
    .rare_active             = .rare_active,
    .display_args            = .display_args,
    .threshold_grade_args    = .threshold_grade_args)
