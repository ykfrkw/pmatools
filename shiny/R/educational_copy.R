...
EDU_COPY <- list(
  ...
  domains = list(
    ...
    rob = list(
      ...
      how      = function(inflation_threshold = 0.10,
        ...
        paste0(
          ...
          "(5) zones differ across the null (above <-> below) -> rate down 1. ",
          "Rule 5 rated down 2 up to pmatools 0.4; it no longer does. Core ",
          "GRADE 4 describes no automatic two-level downgrade for risk of ",
          "bias - every leaf of its Figure 2 reads 'rate down' or 'do not ",
          "rate down' - so every automated risk-of-bias judgment here is ",
          "capped at one level. Two levels remain available, but only as an ",
          "explicit override with a written rationale, below. ",
    ...
    imprecision = list(
      ...
      how      = paste0(
        "Imprecision follows Core GRADE 2 Figure 4, which begins with a ",
        "single question: does the pooled 95 percent confidence interval ",
        "cross the chosen threshold? ",
        "YES - rate down one level. Consider two levels if the CI crosses ",
        "two thresholds (important benefit and important harm), or if the ",
        "most appropriate plain language summary warrants 'may' rather than ",
        "'likely'. Sample size is NOT considered on this path: the Optimal ",
        "Information Size is never consulted. ",
        "NO - the magnitude of the effect decides. A moderate effect: do not ",
        "rate down. An implausibly large effect: proceed to the OIS. ",
        "The OIS is therefore reached only when the CI does not cross the ",
        "threshold AND the effect is implausibly large. The pre-0.5 rule that ",
        "a total sample below 30 percent of the OIS forced two levels ",
        ...
        "Figure 4's second two-level condition - whether the plain language ",
        "summary warrants 'may' rather than 'likely' - is a reviewer ",
        "judgment about wording and is not assessed automatically; apply it ",
        "through the override below. ",
    ...
    pubias = list(
      ...
      how      = paste0(
        ...
        "and the headings on this tab use the same numbering. ",
        "Q1: are most or all studies small AND industry-sponsored? Yes -> ",
        "rate down 1 (some concerns). ",
        "Q2: is statistical analysis of publication bias feasible - that is, ",
        "a meta-analysis was performed with at least 10 studies? Evaluated ",
        "automatically from the number of studies contributing a usable ",
        "estimate. ",
        "Q3 (k >= 10): does funnel plot asymmetry, visually or by test, ",
        "strongly suggest publication bias? The contour-enhanced funnel plot ",
        "is shown with Egger's linear regression. Yes -> rate down 1; no -> ",
        "do not rate down. The reviewer can override Egger with a visual ",
        "judgment. Figure 5 asks this node qualitatively and names no ",
        "p-value: the operational cut-off of p < 0.05 used here is a pmatools ",
        "convention, not a Core GRADE criterion. There is no second tier. ",
        "Core GRADE 4 never rates down two levels for publication bias, so ",
        "the two-tier Egger rule pmatools applied before 0.5 - a stricter ",
        "p-value that rated down two levels - was removed, and the judgment ",
        "is now at most one level down either way. ",
        "Q4 (k < 10): is there documentation of unpublished studies (eg, in a ",
        "trial registry or FDA records)? Yes -> rate down 1. ",
        "One further input on this tab - 'Overall, does the situation argue ",
        "against reporting bias?' - is NOT a node of Figure 5. It is a ",
        "pmatools convenience input, and pmatools 0.5 moved its effect to ",
        "after Q1 rather than treating it as an entry rule-out, so a body of ",
        "small industry-sponsored trials still rates down even when registry ",
