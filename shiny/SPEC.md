...
The card opens with **a load banner** — `pma_banner(tone = "success")` carrying
`pma_load_summary()` ("36 rows, 18 studies, long format.") — because the step's
job is to answer "is my data right?", not to display it. It replaces a
`verbatimTextOutput` reading `Status: ...`; a read failure renders the same
banner in its warning tone instead.

Which column filled which role is not stated here: Step 2's column mapping
shows every role's source column in a select the reviewer can change, so a
read-only restatement of the same detection above the preview table was
duplicated noise rather than a second opinion.
...
**The render is self-sufficient, and it does not assume it runs after the clear/reseed observer.** An earlier version of this section claimed the opposite — that the observer, being created earlier in `step3_server()`, "runs first in the flush". **That premise is false.** Both the observer and this output depend on `question_state()`; Shiny invalidates a reactive's dependents in the *lexicographic* order of their context ids, which creation order does not determine. When the render won, it seeded the box from the previous question's reactiveVal under `isolate()`, the observer then set that reactiveVal to `NA`, and nothing invalidated the output a second time — so a reviewer who clicked **Non-inferiority** on the default question was left with `50` still in a box headed *Non-inferiority threshold*, above a status line reading *"No non-inferiority threshold is set"*, beside a disabled Next. Server state was right throughout; only the HTML was wrong.

What makes it self-sufficient is the **seed key**, not an ordering. `step3_threshold_seed_key(ma_obj, question)` (`R/step3_grade.R`, file scope) is `class(ma_obj)[1]` / `sm` / `question`, and `threshold_seed_key()` holds the key the stored thresholds were last seeded under. `.abs_value()` / `.rel_value()` offer a stored value **only** when that key is the one the render is building for; otherwise the value belongs to a different question or a different analysis and they fall through to the prefill rule — the suggestion on a superiority question, an empty box on a margin one. So the panel cannot display a number stamped for something else whichever of the two runs first. `test-step3-question-panel.R` is the standing guard: it drives the app's own `renderUI` from an observer of higher priority than the seeder, which is what "the render won the race" is.
...
**Both directions go through `.seed_thresholds()`**, and that is what the *question* in the seed key buys: a change of question makes the stored key stale on its own, so the clear is that function's own "fresh seed" branch and no second reading of the prefill rule is needed to decide what follows it.

- **into** a margin question → cleared, and nothing offered in its place, because `prefill` is `FALSE` for both margin questions;
- **out of** one → cleared, *then* re-seeded, because the clear has removed the margin the reviewer typed and *"only ever prefill a reactiveVal that is still NA"* would otherwise decline to re-offer the suggestion over it;
- either way → the key is stamped with the new question, which is what lets `output$threshold_panel` tell a margin typed under *this* question from a leftover belonging to the previous one; an earlier version cleared the two values by hand on the way in and left the key naming the old question, and that unstamped key is what let the panel go on displaying the suggestion;
- either way → `threshold_confirm` is unticked (§3.4.13).

The one case `.seed_thresholds()` cannot key is `state$ma == NULL` — there is no analysis to name — and it voids the key and both threshold values outright rather than returning, so a question changed while Step 2 is momentarily empty cannot carry the previous question's number into the new one either.

**The control-group risk is deliberately carried across the reseed.** On a question change the observer reads `threshold_baseline_state()` *before* calling `.seed_thresholds()` and writes it back after it returns. Without that it is lost: `.seed_thresholds()` clears all three threshold reactiveVals — the baseline included — whenever the seed key goes stale, and a change of question is now exactly that. The control-group risk is a property of the **outcome's data**, not of the question asked about it, and a reviewer who replaced the pooled value and justified the replacement in writing (`baseline_rationale_ok()`, itself a Next gate) must not have that thrown away by a click on a radio one section above it. The `.seed_thresholds()` clear still runs, because a *fresh analysis* must discard the previous outcome's margin whatever the question is; what is restored is only the answer the question change had no business touching.
