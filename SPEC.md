...
Keys are **stable API**: a consumer branches on `key`, not on the wording of `label` or `value`. Indirectness and Publication bias can adopt the container later with no change to the accessor or the renderers.
