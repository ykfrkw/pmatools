...
**Vocabulary: "MID" is internal, "threshold" is what users read [v0.5.1].** The argument value stays `threshold_type = "mid"`, and so do the internal names built on it (`has_mid`, `mid_zone`, `mid_suffix`, `ois_delta`) — renaming them would break callers for a word. But every string a user *reads* says **threshold**: the five `Rating target: …` notes from `.resolve_auto_target()`, the `.check_threshold_type_gate()` abort and its `suggest_threshold()` hint, and the Imprecision Fig 4 labels (`the +/-Threshold band`, `BOTH Thresholds (+/-Threshold)`, `no Threshold was supplied`, `an RRR rather than the threshold`). The strings `threshold_sides = "worse_only"` adds keep the rule: `the Threshold on the worse side (+Threshold)`, `crosses the Threshold on the worse side`, `stays inside the Threshold on the worse side`, `entirely beyond the Threshold on the worse side`, `.check_threshold_sides()`'s two aborts, and — in the two plain-language families of §5.5c — `the equivalence threshold` and `the non-inferiority threshold`. **No cell of `PLAIN_LANGUAGE_FRAMES` may contain the word, and no string literal anywhere in `R/` may either; `test-plain_language.R` holds a standing guard on each.** The second guard is a source scan rather than a call to every builder, because a string literal cannot be reached by a test that does not know which argument combination renders it, and a rule about a forbidden word has to fail where the word is typed. It found the one that had escaped: the Chinn/responder footnote in `.chinn_note()` described Core GRADE 6's option 2 as computing "the proportion in each arm improving by more than the MID" two sentences after its own *"Threshold definition:"* clause — in the same footer, on the same table. Comments and roxygen are excluded from the scan on purpose: the internals keep the name, and the roxygen occurrences that remain are the verbatim quotations the exception below covers. The two words name one thing, the app's Configuration tab calls it a threshold, and a reviewer meeting both in one notes string had to work out that they were not two. The one exception is text quoted **verbatim** from Core GRADE, which keeps the source's own word — e.g. the gate hint's *"MIDs associated with mortality of 1%…"*.
...
- **The signal is the `"Manual override (<judgment>): <rationale>"` head** that `make_domain_row()` writes, parsed back by `.parse_override_note()` in `R/domain_row.R`, **not** `auto == FALSE`. `auto == FALSE` also means "the reviewer supplied an input the algorithm cannot compute" — an answered `pubias_small_industry`, say — where the flowchart still decided the judgment and the facts still explain it. `auto` is checked as a corroborating guard, so a domain note that merely quotes the phrase does not trigger it.
