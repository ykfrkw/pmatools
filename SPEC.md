...
**Two-row header [v0.5.1].** The layout has two header rows, but only the three absolute-effect columns actually use both. Every other column — outcome, participants, relative effect, certainty, and the plain language summary when present — is **merged vertically across both rows**, with its label in the top row (a flextable span renders its top-left cell, so a blank top cell would erase the label). Before this the top row left those five cells empty; the header background is one solid navy, so the blanks read as a full-width band with "Absolute effects (95% CI)" floating on it rather than as a heading over three columns. The header is bottom-aligned so the seven column labels share a baseline. The merge survives into the `.docx` as `w:vMerge`, and the spanning row as `w:gridSpan`.
...
**The three proportions are computed once [v0.6].** `.chinn_rates(meta_obj, baseline_risk, invert)` returns `p1` / `p1_lo` / `p1_hi` and the odds-ratio triple they came from, or `NULL` when any ingredient is missing. Four cells of the row are functions of exactly those numbers — both arm cells, the Difference and the derived risk ratio — and each of them re-deriving the conversion is how they end up disagreeing after one is edited. `.format_ier_chinn()` is a formatter over it and its output is unchanged.
...
**The Effect column gains the derived risk ratio [v0.6].** A second line under the pooled estimate: *"Derived risk ratio 2.57 (2.31 to 2.79)"* — `p1 / baseline_risk`, with bounds from `p1_lo` and `p1_hi`. A second line and not a second column: it is the same pooled estimate read on another scale, and a column would present it as an independent result. Rendered in both layouts, with the layout's own CI separator.

**Both are DERIVED, and the footnote says so.** `.chinn_note()` carries one sentence naming the risk difference and the risk ratio, stating that both come from the pooled estimate through Chinn's formula rather than from a fit, that they carry its logistic latent-variable assumption, and that they move with the assumed control proportion — which it names (`"an assumed control responder proportion of 0.3"`). In a combined table the proportion can differ per row, so it is named in the per-row note (§4.9) instead.

**`keep_effect_scale = TRUE` shows both presentations in one row [v0.6].** Read only when `convert_smd_to_or = TRUE`. This is the presentation Core GRADE 6 recommends. **One row, two lines per cell** — no extra rows, no extra columns:

- each arm cell holds the mean-scale value (§ "Arm-level columns", including its interval for the intervention arm) on its first line(s) and the responder proportion on the last;
- the Difference cell holds the standardised mean difference first and the per-`per` risk difference second, in that order, so a reader reading across the row stays on one line;
- the arm headers are the measure-neutral **"With control" / "With intervention"** in both layouts. The rate wording and the `per` denominator would head a cell whose first line is a mean and whose denominator applies to only half of it; "With control" is the one wording that describes both lines. (The BMJ layout uses that wording unconditionally, so only the GRADEpro headers switch.)
- the footer carries **both** derivation notes: the arm-level one (`.cont_arm_note()`) and the Chinn one, plus the sentence saying the derived quantities are derived.

**Degradation.** `.format_arm_values_cont()` returns `NULL` when it cannot produce honest numbers — no usable control-arm means, no usable pooled estimate, or (for an SMD) no reference SD to re-express it with. In `keep_effect_scale` mode the row then **degrades to the responder-only presentation**, never to an empty cell and never to an error: the arm cells and the Difference cell both fall back, so half the row cannot show one presentation while half shows the other, and a footnote names the reason (*"Both the effect on its own scale and the proportion of responders were asked for; the arm columns show the responder proportions only, because …"*).
